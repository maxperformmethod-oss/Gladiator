...
/**
 * Centralizovaná dátová vrstva nad localStorage. Všetky dáta jedného člena
 * žijú pod jedným verzovaným, na člena viazaným kľúčom.
 *
 * A1 (oproti MAXPERFORMu): Gladiator má účty a na jednom prehliadači sa môžu
 * prihlásiť viacerí. Kľúč preto obsahuje `clenId` – iný člen na tom istom
 * zariadení NIKDY nevidí cudzie dáta. Toto je bezpečnostná požiadavka.
 */
export const DATA_VERSION = 2 as const

/** Kľúč lokálnych dát člena: `gladiator:klub:v1:<clenId>`. */
export function storageKey(clenId: string): string {
  return `gladiator:klub:v1:${clenId}`
}

export function defaultPreferences(): Preferences {
  return { restSec: 90, soundOn: true, weeklyGoal: 3 }
}
...
/* ---------- čítanie a zápis (na člena viazaný kľúč) ---------- */
