...
import { newRecordsInSession } from '@/lib/klub/calc'
import { formatDurationWords, formatKg, formatLongDate, formatTime, plural } from '@/lib/klub/format'
  ...
  if (!session) {
    return (
      <div className="mx-auto max-w-lg pt-10 text-center">
        ...
        <Button className="mt-5" onClick={() => router.push('/klub/historia')}>
          Späť na históriu
        </Button>
      </div>
    )
    ...
    >
      ...
      <div className="mb-5 flex items-center gap-3">
        <button
          ...
          onClick={() => router.push('/klub/historia')}
          className="flex size-11 shrink-0 items-center justify-center rounded-xl border border-line bg-surface text-ink-dim hover:text-ink"
        >
      ...
      {isSummary && (
        <div className="mt-6 flex gap-3">
          <Button variant="secondary" className="flex-1" onClick={() => router.push('/klub/historia')}>
            História
          </Button>
          <Link href="/klub" className="flex-1">
