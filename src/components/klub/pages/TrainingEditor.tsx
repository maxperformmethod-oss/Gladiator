...
import { ArrowDown, ArrowLeft, ArrowUp, Check, Dumbbell, Plus, Trash2, X } from 'lucide-react'
...
import { previousPerformance } from '@/lib/klub/calc'
import { formatKg, formatRelativeDay, plural } from '@/lib/klub/format'
import { uid } from '@/lib/klub/id'
import { MUSCLE_GROUPS, MUSCLE_GROUP_LABELS, type PlanExercise, type WorkoutPlan } from '@/lib/klub/types'
...
export default function TrainingEditor({ id }: { id?: string }) {
  const isNew = !id
  const { data, savePlan, katalog } = useApp()
  ...
  const [exercises, setExercises] = useState<PlanExercise[]>(
    existing?.exercises.map((e) => ({ ...e, sets: e.sets.map((s) => ({ ...s })) })) ?? [newExercise()],
  )
  ...
  if (!isNew && !existing) {
    return (
      <div className="mx-auto max-w-lg pt-10 text-center">
        ...
        <Button className="mt-5" onClick={() => router.push('/klub/trening')}>
          Späť na tréningy
        </Button>
      </div>
    )
  ...
  const addSet = (ex: PlanExercise) => {
    ...
    patchExercise(ex.id, {
      sets: [...ex.sets, { id: uid(), reps: last?.reps ?? 8, weight: last?.weight ?? 0 }],
    })
  }
    ...
    >
      <div className="mb-6 flex items-center gap-3">
        <button
          ...
          onClick={() => router.push('/klub/trening')}
        ...
        <h1 className="display text-2xl text-ink">{isNew ? 'Nový tréning' : 'Upraviť tréning'}</h1>
      </div>
              ...
              >
                <Card>
                  <div className="mb-3 flex items-center justify-between gap-2">
                    <span className="flex size-7 items-center justify-center rounded-lg bg-gold/12 text-xs font-extrabold text-gold-hi">
                      {index + 1}
                    </span>
                    <div className="flex gap-1">
                      <button
                        ...
                        className="flex size-11 items-center justify-center rounded-lg text-ink-dim hover:bg-surface-3 hover:text-ink disabled:opacity-30"
                      >
                  ...
                  <div className="grid gap-3 sm:grid-cols-[auto_1fr]">
                    <div>
                      ...
                      <select
                        ...
                        className="h-11 w-full min-w-36 rounded-xl border border-line-strong bg-surface-3 px-3 text-base font-medium text-ink outline-none focus:border-gold sm:w-auto"
                      >
                    ...
                    {ex.sets.map((set, setIndex) => (
                      <div key={set.id} className="flex items-end gap-2">
                        ...
                        <button
                          ...
                          className="mb-0.5 flex size-11 shrink-0 items-center justify-center rounded-lg text-ink-faint hover:bg-danger/10 hover:text-danger"
                        >
