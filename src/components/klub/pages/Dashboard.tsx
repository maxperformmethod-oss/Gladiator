...
import { Dumbbell, Flame, Play, Target, Trophy, Zap } from 'lucide-react'
import { useApp } from '../AppProvider'
import { Button } from '../ui/Button'
import { Card } from '../ui/Card'
import { GoalRing } from '../dashboard/GoalRing'
import { WeekStrip } from '../dashboard/WeekStrip'
import { currentStreak, estimateCalories, personalRecords, startOfWeek } from '@/lib/klub/calc'
import {
  formatDurationWords,
  formatKg,
  formatKgCompact,
  ...
  greeting,
  plural,
} from '@/lib/klub/format'
import type { AppData, WorkoutPlan } from '@/lib/klub/types'

/** Odporučí dnešný plán: ten, ktorý sa netrénoval najdlhšie. */
function suggestPlan(data: AppData): WorkoutPlan | null {
...
export default function Dashboard() {
  ...
  return (
    ...
    >
      ...
      <h1 className="display mt-1.5 text-2xl text-ink sm:text-3xl">
        {greeting()}. {streak > 0 ? 'Séria beží – nezastavuj.' : 'Pripravený začať?'}
      </h1>

      {/* Hero: dnešný tréning – dominantné CTA */}
      <Card tone="gold" className="mt-7">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <p className="text-xs font-semibold uppercase tracking-wider text-gold-hi">
            ...
            <div className="flex items-center gap-3">
              <span
                className={`flex size-10 shrink-0 items-center justify-center rounded-xl ${
                  streak > 0 ? 'bg-gold/15 text-gold-hi' : 'bg-surface-3 text-ink-faint'
                }`}
              >
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        ...
        <Card>
          <div className="mb-3 flex items-center justify-between">
            ...
            {last && (
              <Link
                href={`/klub/historia/${last.id}`}
                className="text-xs font-semibold text-gold-hi hover:underline"
              >
                Detail
              </Link>
            )}
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        ...
        <Card>
          <div className="flex items-start gap-3">
            <span className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-gold/15 text-gold-hi">
