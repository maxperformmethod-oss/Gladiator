...
import { exerciseRecords } from '@/lib/klub/calc'
import { formatDate, formatKg, plural } from '@/lib/klub/format'
import { MUSCLE_GROUP_LABELS } from '@/lib/klub/types'
...
export default function Records() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
      <h1 className="display text-2xl text-ink">Osobné rekordy</h1>
            ...
            <Card key={row.exercise}>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                <div className="rounded-xl border border-gold/25 bg-gold/8 p-3">
                  <p className="text-[10px] font-bold uppercase tracking-wider text-gold-hi">
                    Odhad 1RM
                  </p>
                  {row.best1RM ? (
                    <>
