...
import { dayKey, formatDurationWords, formatKg, formatRelativeDay, formatTime, plural } from '@/lib/klub/format'
...
export default function History() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      <h1 className="display text-2xl text-ink">História tréningov</h1>
        ...
        <div className="space-y-3">
          {visibleSessions.length === 0 ? (
            ...
            visibleSessions.map((s) => (
              <Link key={s.id} href={`/klub/historia/${s.id}`} className="block">
