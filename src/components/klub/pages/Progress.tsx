...
import { MAX_COMPARE_EXERCISES, SERIES_COLORS } from '../charts/chartColors'
import { epley1RM, personalRecords, startOfWeek, volumeByMuscleGroup } from '@/lib/klub/calc'
import { formatDate, formatKg, plural } from '@/lib/klub/format'
import { MUSCLE_GROUP_LABELS, UNASSIGNED_MUSCLE_GROUP_LABEL } from '@/lib/klub/types'
import type { WorkoutSession } from '@/lib/klub/types'

/** Grafy sú ťažké (recharts) – lenivo načítané a bez SSR (potrebujú okno). */
const ChartSkeleton = () => (
  <div className="h-48 animate-pulse rounded-xl bg-surface-3" aria-hidden />
)
const CompareChart = dynamic(() => import('../charts/ProgressCharts').then((m) => m.CompareChart), {
  ssr: false,
  loading: ChartSkeleton,
})
const VolumeBars = dynamic(() => import('../charts/ProgressCharts').then((m) => m.VolumeBars), {
  ssr: false,
  loading: ChartSkeleton,
})
const CountBars = dynamic(() => import('../charts/ProgressCharts').then((m) => m.CountBars), {
  ssr: false,
  loading: ChartSkeleton,
})
const MuscleBars = dynamic(() => import('../charts/ProgressCharts').then((m) => m.MuscleBars), {
  ssr: false,
  loading: ChartSkeleton,
})
...
export default function Progress() {
  ...
  const filtered = useMemo(() => filterByPeriod(data.sessions, periodDays), [data.sessions, periodDays])
  ...
  const muscleVolume = useMemo(
    () =>
      volumeByMuscleGroup(filtered).map((row) => ({
        label: row.group === 'nezaradene' ? UNASSIGNED_MUSCLE_GROUP_LABEL : MUSCLE_GROUP_LABELS[row.group],
  ...
  if (data.sessions.length === 0) {
    return (
      <div>
        <h1 className="display text-2xl text-ink">Progres</h1>
        <div className="mt-6">
          <EmptyState
            icon={ChartLine}
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
          ...
          {PERIODS.map((p) => (
            <button
              ...
              className={`h-9 rounded-lg px-3 text-xs font-bold transition-colors ${
                period === p.key ? 'bg-gold text-bg' : 'text-ink-dim hover:text-ink'
              }`}
            >
              {p.label}
            </button>
      ...
      <Card className="mt-5">
          ...
          <div className="flex rounded-xl border border-line bg-surface-3 p-1" role="group" aria-label="Zobrazovaná metrika">
            <button
              ...
              className={`h-8 rounded-lg px-2.5 text-xs font-bold transition-colors ${
                metric === 'weight' ? 'bg-gold text-bg' : 'text-ink-dim hover:text-ink'
              }`}
            >
              Max váha
            </button>
            <button
              ...
              className={`h-8 rounded-lg px-2.5 text-xs font-bold transition-colors ${
                metric === '1rm' ? 'bg-gold text-bg' : 'text-ink-dim hover:text-ink'
              }`}
            >
              Odhad 1RM
            </button>
        ...
        {hasEnoughData ? (
          <CompareChart data={compareSeries} selectedExercises={selectedExercises} />
        ) : (
      ...
      <div className="mt-3 grid gap-3 lg:grid-cols-2">
        ...
        <Card>
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <Link href="/klub/rekordy" className="text-xs font-semibold text-gold-hi hover:underline">
              Všetky
            </Link>
          </div>
          {records.length > 0 ? (
