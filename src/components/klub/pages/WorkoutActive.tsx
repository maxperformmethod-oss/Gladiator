...
import { activeProgress, previousPerformance } from '@/lib/klub/calc'
import { formatDuration, formatKg } from '@/lib/klub/format'
...
export default function WorkoutActive() {
  ...
  const doFinish = () => {
    ...
    if (session) router.replace(`/klub/historia/${session.id}?summary=1`)
  }
      ...
      <div className="sticky top-14 z-20 -mx-4 mb-5 border-b border-line bg-bg/90 px-4 py-3 backdrop-blur-md sm:-mx-6 sm:px-6 lg:top-0 lg:mx-0 lg:rounded-2xl lg:border lg:px-5">
        <div className="flex items-center justify-between gap-3">
          <button
            ...
            onClick={() => router.back()}
            className="flex size-11 shrink-0 items-center justify-center rounded-xl text-ink-dim hover:bg-surface-2 hover:text-ink"
          >
            <ChevronLeft className="size-5" aria-hidden />
          </button>
          <div className="min-w-0 flex-1 text-center">
            <h1 className="truncate text-base font-extrabold">{active.name}</h1>
            <p className="tnum mt-0.5 text-xs font-semibold text-gold-hi" role="timer">
          ...
          <button
            ...
            className="flex size-11 shrink-0 items-center justify-center rounded-xl text-ink-dim hover:bg-danger/10 hover:text-danger"
          >
      ...
      <div className="space-y-3">
        {active.exercises.map((ex, exIndex) => {
          ...
          return (
            <Card
              key={ex.id}
              tone={isCurrent && !exDone ? 'gold' : 'default'}
            ...
            >
              <div className="mb-1 flex items-center justify-between gap-2">
                ...
                {isCurrent && !exDone && (
                  <span className="shrink-0 rounded-md bg-gold/15 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-gold-hi">
                    Aktuálny
                  </span>
                )}
              </div>
